...
class Instructions():
    def __init__(self, assets: Any, screen: pygame.Surface) -> None:
        ...
        self.background = pygame.transform.scale(
            assets.background2, (self.width, self.height))
        ...
        self.cheats_text = [
            ...
            "  LEVEL SKIP",
            "  GHOST FREEZ",
            "  +1 ONE LIFE (7 IS THE LIMIT)",
            "  INCREAS PACMANS SPEED",
            "  STOP TIME"
        ]
        self.buttons = [("EXIT TO MAIN MENU", GameState.MENU),
                        ("HIGHSCORES", GameState.HIGHSCORES)]
    ...
    def _draw_keys(self) -> None:
        coords = [
            ...
            (self.width * 0.125, self.height * 0.48),
            (self.width * 0.125, self.height * 0.61)
        ...
        for i, coord in enumerate(coords):
            box_rect = pygame.Rect(
                    coord[0], coord[1],
                    self.width * 0.08, self.height * 0.05)
            if i < 4:
    ...
    def _draw_inst_text(self) -> None:
        coords = [
            ...
            (self.width * 0.25, self.height * 0.63),
        ]
    ...
    def run(self) -> None:
        ...
        pygame.draw.rect(
            overlay, (0, 0, 0, 220), overlay.get_rect(), border_radius=50)
        self.screen.blit(overlay, (self.width * 0.02, self.height * 0.02))
        pygame.draw.rect(
            self.screen, "darkblue", box_rect, width=10, border_radius=50)
        self.screen.blit(
            label_surface, (self.width * 0.35, self.height * 0.10))
        self.screen.blit(
            label_surface2, (self.width * 0.35, self.height * 0.12))
...
class HighScores():
    ...
    def enter_name(self, done: bool, score: int) -> None:
        ...
        self.screen.blit(
            label_surface2, (self.width * 0.44, self.height * 0.60))
        spacing = 0.375
    ...
    def run(self) -> None:
        ...
        overlay = pygame.Surface(
            (box_rect.width, box_rect.height), pygame.SRCALPHA)
        pygame.draw.rect(
            overlay, (0, 0, 0, 200), overlay.get_rect(), border_radius=50)
        self.screen.blit(overlay, (self.width * 0.02, self.height * 0.02))
        pygame.draw.rect(
            self.screen, "darkblue", box_rect, width=10, border_radius=50)
        self.screen.blit(
        ...
        for i, highscore in enumerate(self.scores):
            name = highscore["name"].upper()
            score = highscore["score"]
            text = f"★☆★ {name} : {score}"
            label_surface = self.font4.render(text, True, "white")
            self.screen.blit(label_surface, (start_x, start_y))
            spacing += 0.11
            start_y = self.height * spacing
...
class Paused():
    def __init__(self, assets: Any, screen: pygame.Surface) -> None:
        ...
        self.paused_list = [("RESUME", GameState.PLAYING),
                            ("EXIT TO MAIN MENU", GameState.MENU)]
        self.list = self.paused_list
        self.index = 0
        self.font = assets.font_20
    ...
    def run(self) -> None:
        ...
        for i, (label, state) in enumerate(self.paused_list):
            box_rect = pygame.Rect(start_x,
                                   start_y + i * (box_height + spacing),
                                   box_width, box_height)
            color = "darkblue"
            if i == self.index:
                color = "white"
